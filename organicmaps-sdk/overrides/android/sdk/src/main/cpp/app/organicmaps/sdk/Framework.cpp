...
JNIEXPORT void Java_app_organicmaps_sdk_Framework_nativeSetEdgeZUserMarkers(JNIEnv * env, jclass clazz,
                                                                           ...
                                                                           jdoubleArray longitudes)
{
  ...
  if (latitudes == nullptr || longitudes == nullptr)
    return;

  jsize const latCount = env->GetArrayLength(latitudes);
  jsize const lonCount = env->GetArrayLength(longitudes);
  jsize const count = std::min(latCount, lonCount);
  if (count <= 0)
    return;

  std::vector<jdouble> latValues(static_cast<size_t>(count));
  std::vector<jdouble> lonValues(static_cast<size_t>(count));
  env->GetDoubleArrayRegion(latitudes, 0, count, latValues.data());
  env->GetDoubleArrayRegion(longitudes, 0, count, lonValues.data());

  editSession.SetIsVisible(UserMark::Type::COLORED, true);
  for (jsize i = 0; i < count; ++i)
  {
    double const lat = static_cast<double>(latValues[static_cast<size_t>(i)]);
    double const lon = static_cast<double>(lonValues[static_cast<size_t>(i)]);
    if (!mercator::ValidLat(lat) || !mercator::ValidLon(lon))
      continue;

    auto * mark = editSession.CreateUserMark<ColoredMarkPoint>(mercator::FromLatLon(lat, lon));
    mark->SetRadius(9.0f);
  }
}
